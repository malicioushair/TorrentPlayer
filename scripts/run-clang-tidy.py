...
def macos_sdk_extra_args():
    ...
    return [
        "--extra-arg=-isysroot",
        f"--extra-arg={sdk_path}",
        "--extra-arg=-isystem",
        f"--extra-arg={sdk_path}/usr/include/c++/v1",
    ]
...
def main():
    ...
    extra_args = macos_sdk_extra_args()
    ...
    for file_path in files:
        ...
        result = subprocess.run(
            [clang_tidy, "-p", str(build_dir), "--quiet", *extra_args, str(file_path)],
            check=False,
        )
